/*
 *  vbHmmTemplate_Main.cpp
 *  Model-specific main function for VB-HMM-TEMPLATE.
 *
...
int main( int argc, char *argv[] ){
    ...
    if( argc < 7 ){
        ...
        cerr << endl << "Variational-Bayesian Hidden-Markov-Model Analysis Template" << endl;
        cerr << "    built on 2018.11.xx" << endl;
        cerr << "Syntax : vbHmmTemplate sFrom sMax trials maxIteration threshold data_filename ..." << endl;
        ...
        cerr << "Example: vbHmmTemplate 2 20 5 1000 1e-10 data-file" << endl << endl;
        freeRan();
        exit(1);
    ...
    // Set parameters
    vbHmmCond_template *c = new vbHmmCond_template();
    c->sFrom = atoi( argv[1] );
    c->sTo = atoi( argv[2] );
    ...
    int i;
    for( i = 6 ; i < argc ; i++ ){
        vbHmmTrace_template *t = new vbHmmTrace_template();
        t->importDataFromFile( argv[i], &logFS );
        if( t->N > 0 ){
            d->appendTrace( t );
        } else {
            delete t;
    ...
    if( d->R > 0 ){
        int optK;
        vbHmmModel_template *m = new vbHmmModel_template();
